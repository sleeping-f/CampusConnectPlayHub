...
/* 1) --- POST send request ---
   ...
   Body (preferred): { student_id_2 }
   Also accepted: { friend_id } or { targetUserId }
...
/* 2) --- PUT respond to request ---
   ...
   Also accepted: { fromUserId } as student_id_1 (compat)
*/
...
router.delete('/:friendId(\\d+)', requireDb, authenticateToken, async (req, res) => {
  try {
    ...
    if (!Number.isInteger(other_id) || other_id <= 0) {
      return res.status(400).json({ message: 'friendId must be a positive integer' });
    }
    if (other_id === me) {
