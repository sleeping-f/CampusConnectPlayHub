...
export default function ReceiveNotifications() {
  ...
  // auth header (matches how StudentInfo does it)
  ...
  }, [page, unreadOnly, API_BASE]); // headers is stable given localStorage, safe to omit
  ...
  const markRead = async (id) => {
    try {
      await axios.patch(`${API_BASE}/api/notifications/${id}/read`, null, { headers });
      setItems(prev =>
  ...
  const markAllRead = async () => {
    try {
      await axios.patch(`${API_BASE}/api/notifications/read-all`, null, { headers });
  ...
  const markUnread = async (id) => {
    try {
      await axios.patch(`${API_BASE}/api/notifications/${id}/unread`, null, { headers });
      setItems(prev =>
        prev.map(n =>
  ...
  const markAllUnread = async () => {
    try {
      await axios.patch(`${API_BASE}/api/notifications/unread-all`, null, { headers });
          ...
          <>
            <ul className="notif-list">
              {items.map(n => {
                ...
                // Compute message first, then title (avoid self-reference bug)
                  ...
                  <li key={n.notification_id} className={`notif-card ${!n.is_read ? 'is-unread' : ''}`}>
                    ...
                    <div className="notif-main">
                      ...
                      {!n.is_read ? (
                        ...
                        <div className="notif-actions">
                          <button className="mark-read" onClick={() => markUnread(n.notification_id)}>
                            Mark unread
                          </button>
                        </div>
                      )}
